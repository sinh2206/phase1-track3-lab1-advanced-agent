...
@app.command()
def main(dataset: str = "data/hotpot_mini.json", out_dir: str = "outputs/sample_run", reflexion_attempts: int = 3, mode: str = "mock", model: Optional[str] = None, base_url: Optional[str] = None, temperature: float = 0.0) -> None:
    if mode not in {"mock", "llm"}:
        raise typer.BadParameter("mode must be 'mock' or 'llm'")
    examples = load_dataset(dataset)
    runtime = MockRuntime() if mode == "mock" else LLMRuntime(model=model, base_url=base_url, temperature=temperature)
